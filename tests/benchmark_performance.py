...
def generate_random_players(n: int) -> List[Player]:
    return [
        Player(
            id=f"P{i}", name=f"Airline {i}", cost=float(np.random.randint(1000, 4000))
        )
        for i in range(n)
    ]
...
def benchmark_scaling():
    ...
    for n in player_counts:
        ...
        # Benchmark Exact
        try:
            config_exact = GameConfiguration(
                players=players, algorithm=AlgorithmType.EXACT
            )
        ...
        config_approx = GameConfiguration(
            players=players, algorithm=AlgorithmType.APPROXIMATE, num_samples=2000
        )
...
def benchmark_convergence():
    ...
    for s in sample_sizes:
        config_approx = GameConfiguration(
            players=players, algorithm=AlgorithmType.APPROXIMATE, num_samples=s
        )
        result_approx = engine.run_simulation(config_approx)
        approx_values = np.array(
            [
                result_approx.shapley_values[pid]
                for pid in result_exact.shapley_values.keys()
            ]
        )
